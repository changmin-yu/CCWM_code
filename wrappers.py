...
class Collect:
  ...
  def step(self, action, truncation_ratio=0.0):
    ...
    if done:
      ...
      if truncation_ratio > 0:
        episode['discount'][-int(truncation_ratio*len(episode['discount'])):] = 0
...
class TruncationDone:
  ...
  def step(self, action, truncation_ratio=0.0):
    obs, reward, done, info = self._env.step(action, truncation_ratio)
    obs['reward'] = reward
    return obs, reward, done, info
...
class Async:
  ...
  def step(self, action, blocking=True, truncation_ratio=0.0):
    return self.call('step', action, blocking=blocking, truncation_ratio=truncation_ratio)

  def reset(self, blocking=True, truncation_ratio=0.0):
